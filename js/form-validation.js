...
(function() {
  ...
  /**
   * Submit form using EmailJS (NO FormSubmit)
  ...
  async function submitFormAjax(form) {
    try {
      ...
      if (!window.AltorraEmailService) {
        console.error('❌ AltorraEmailService not loaded');
        throw new Error('Sistema de correo no disponible');
      }
      ...
      if (form.id === 'publishForm') {
        formType = 'publishForm';
      } else if (form.id === 'detailContactForm' || form.classList.contains('detail-contact')) {
        formType = 'detailForm';
      }

      // Process form with EmailJS
      console.log('📧 Enviando formulario con EmailJS...');
      const result = await window.AltorraEmailService.processForm(formData, formType);

      // Check result
      if (result.success) {
        // Success!
        console.log('✅ Form submitted successfully:', result.radicado);

        // Announce to screen readers
        announceToScreenReader('Formulario enviado correctamente. Gracias por contactarnos.', form);

        // Show success modal with radicado
        showSuccessModal(result.radicado, result.data);
        ...
        const errorMessage = result.error || 'Error al enviar el formulario. Por favor intenta nuevamente.';
  ...
  /**
   * Muestra modal de confirmación con número de radicado
   * @param {string} radicado - Número de radicado
   * @param {Object} data - Datos del formulario
   */
  function showSuccessModal(radicado, data) {
    // Remove existing modal if any
    const existingModal = document.querySelector('.altorra-success-modal');
    if (existingModal) existingModal.remove();

    // Create modal overlay
    const modal = document.createElement('div');
    modal.className = 'altorra-success-modal';
    ...
    modal.innerHTML = `
      ...
      <div class="modal-content">
        ...
        <h2 id="modalTitle" class="modal-title">¡Mensaje enviado correctamente!</h2>
        <div class="modal-radicado">
          <strong>Número de radicado:</strong>
          <div class="radicado-number">${radicado}</div>
        </div>
        <p class="modal-text">
          Hemos recibido tu solicitud. Un asesor de Altorra se pondrá en contacto contigo pronto.<br>
          <small style="color: #6b7280; margin-top: 8px; display: block;">
            Revisa tu correo electrónico (${data.Email || data.email || ''}) para ver la confirmación.
          </small>
        </p>
        <button class="modal-close-btn" onclick="this.closest('.altorra-success-modal').remove()" aria-label="Cerrar modal">
          Entendido
        </button>
      </div>
    `;
    ...
    style.textContent = `
      ...
      .altorra-success-modal .modal-overlay {
        position: absolute;
        inset: 0;
        background: rgba(0, 0, 0, 0.5);
        backdrop-filter: blur(4px);
      }
      .altorra-success-modal .modal-content {
        position: relative;
        background: white;
        border-radius: 16px;
        padding: 32px;
      ...
      .altorra-success-modal .modal-icon {
        margin: 0 auto 24px;
        width: 64px;
        height: 64px;
      }
      .altorra-success-modal .modal-title {
        font-size: 1.5rem;
        font-weight: 700;
        color: #111827;
        margin: 0 0 16px;
      }
      .altorra-success-modal .modal-radicado {
        background: #f3f4f6;
        border-radius: 12px;
        padding: 16px;
        margin: 16px 0;
      }
      .altorra-success-modal .radicado-number {
      ...
      .altorra-success-modal .modal-text {
        color: #6b7280;
        line-height: 1.6;
        margin: 16px 0 24px;
      }
      .altorra-success-modal .modal-close-btn {
      ...
      .altorra-success-modal .modal-close-btn:hover {
  ...
  function interceptFormSubmit() {
    document.addEventListener('submit', async (e) => {
      ...
      // SIEMPRE prevenir envío normal (NO más FormSubmit)
      e.preventDefault();

      console.log('📧 Intercepting form submit for EmailJS...');
